<?php
	...
	if($forma_pago == "01" && $monto_contenido >= $limite) {
		...
		if($error > 0) {
			echo "Insert into venta values(null, null, '$fecha', '$hora', '$tipo_pago','Efectivo', $total,$subtotal,$porcentaje,$inDescuento, $id_vendedor, null, $nro_mov ,$recibidoT,$recibido)";
			$res['mens'] = " No se ha podido guar la venta detalle. ".mysqli_error($con);
			echo json_encode($res);
		} else {
